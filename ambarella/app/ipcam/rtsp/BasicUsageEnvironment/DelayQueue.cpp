...
//#include <stdio.h>
void DelayQueue::addEntry(DelayQueueEntry* newEntry) {	//it's a double link list
  synchronize();

  DelayQueueEntry* cur = head();
//  printf("remain %d : %d\n", cur->fDeltaTimeRemaining.seconds(), cur->fDeltaTimeRemaining.useconds());	//jay
...
//  printf("next alarm %d : %d\n", fNext->fDeltaTimeRemaining.seconds(), fNext->fDeltaTimeRemaining.useconds());	//jay
}
...
void DelayQueue::removeAllEntry() {
DelayQueueEntry* next;
  DelayQueueEntry* cur = head();
  while (cur != this) {
  	next = cur->fNext;
	  removeEntry(cur);
