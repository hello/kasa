...
void MediaSubsession::setDestinations(netAddressBits defaultDestAddress) {
  ...
  if (fRTPSocket != NULL) {
	...
	if (fRTCPSocket != NULL) {
		...
		if(IsMulticastAddress(destAddr.s_addr)) {
			//multicast server will always send RTCP packet to the same port as its local port
			fRTCPSocket->changePort(destPort);
			fRTCPSocket->joinGroup(destAddr.s_addr);
		}
		if (isSSM()) {
