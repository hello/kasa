...
RTCPInstance::RTCPInstance(UsageEnvironment& env, Groupsock* RTCPgs,
  ...
  if (fOutBuf == NULL) return;
	...
	printf("RTCP: turnOn: sock %d, port %d\n", RTCPgs->socketNum(), ntohs(RTCPgs->port().num()));		//jay
...
//  printf("setSpecificRRHandler 0x%x : %d, rrHandler %p\n", fromAddress, ntohs(fromPort.num()), rrHandler);	//jay
...
//	  printf("fSpecificRRHandlerTable 0x%x : %d, rrHandler %p\n", fromAddr, ntohs(fromPort.num()), rrHandler);	//jay
			  if (rrHandler != NULL) {
