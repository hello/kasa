...
public:
  ...
  void enqueue(unsigned char const* from, unsigned numBytes) {
  	memmove(curPtr(), from, numBytes);  increment(numBytes); }
  
  void enqueueWord(unsigned word) { unsigned nWord = htonl(word);
  		enqueue((unsigned char*)&nWord, 4); }
		
  void insert(unsigned char const* from, unsigned numBytes, unsigned toPosition) {
	memmove(&fBuf[fPacketStart + toPosition], from, numBytes);
	if (toPosition + numBytes > fCurOffset) 
		fCurOffset = toPosition + numBytes; }
  
  void insertWord(unsigned word, unsigned toPosition) { unsigned nWord = htonl(word);
 ...
 void skipBytes(unsigned numBytes) {  increment(numBytes);}
