...
ar6000_init_module(void)
{
    ...
    printk("init AR6002 driver\n");

#ifdef CONFIG_PLAT_AMBARELLA
	ambarella_set_gpio_output(&ambarella_board_generic.wifi_power, 0);
	ambarella_set_gpio_output(&ambarella_board_generic.wifi_reset, 1);
	ambarella_set_gpio_output(&ambarella_board_generic.wifi_power, 1);
	ambarella_set_gpio_output(&ambarella_board_generic.wifi_reset, 0);
...
/* ATHENV */
    if (dev_alloc_name(dev, "wlan%d") < 0) {
         printk(KERN_CRIT "Could not dev_alloc_name\n");
    }
