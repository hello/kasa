...
#define ATH_LOG_WARN  ATH_DEBUG_WARN

#define ATH_MSG_LEVEL ATH_LOG_TRC

#if DBG
#define HIF_DEBUG_PRINTF(Level, Fmt) \
{ \
    if ((Level) <= ATH_MSG_LEVEL) \
    { \
        printf("HIFReadWrite:"); \
        printf Fmt; \
    } \
...
A_STATUS HIFGetBlockLength(HANDLE hDevice, PDWORD pBlockLength);
A_STATUS HIFSetBlockLength(HANDLE hDevice, DWORD BlockLength);
A_STATUS HIFGetClock(HANDLE hDevice, PDWORD pClock);
A_STATUS HIFSetClock(HANDLE hDevice, DWORD Clock);
A_STATUS HIFGetBusWidth(HANDLE hDevice, char* width);
A_STATUS HIFSetBusWidth(HANDLE hDevice, char width);
