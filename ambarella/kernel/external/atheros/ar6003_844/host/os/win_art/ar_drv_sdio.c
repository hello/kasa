...
/* These are defines for remoteIP.ini settings */
#define DEF_SDIO_HOST_NVMEM_OVERRIDE  "SDIO_HOST_NVMEM_OVERRIDE"
#define DEF_SDIO_HOST_NVMEM_OVERRIDE_DEFAULT   0 
static int sdioNvmemOverride = DEF_SDIO_HOST_NVMEM_OVERRIDE_DEFAULT;
static void parseRemoteIP_ini_SDIO(void);
...
A_BOOL SDIO_InitAR6000_ene(HANDLE *handle, A_UINT32 *nTargetID)
{
    ...
    *handle = hDevice;
	...
	if (sdioNvmemOverride) {
	    /* interact with host_proxy via HOST_INTEREST to control BMI active */
  	    A_UINT32 param;
	    A_UINT32 loop = 0;

	    do{
		    if (ar6000_ReadDataDiag(ar->arHifDevice, 								
				                    TARG_VTOP(TARGET_TYPE_AR6003, AR6003_HOST_INTEREST_ITEM_ADDRESS(hi_refclk_hz)),																		
	                                (A_UCHAR *)&param,
	                                4) != A_OK)	
	        {
	            AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("SDIO_InitAR6000_ene failed at hi_refclk_hz!!! \n"));
	            while(1){};
	        }
	        loop++;
	    }while(param != (26000000));

	    /* this code is called prior to BMIGetTargInfo so we must assume AR6003 */	
	    /* 2: run from serial flash, 3: run from bmi */
	    param = (3);
	
	    if (ar6000_WriteDataDiag(ar->arHifDevice, 
				                 TARG_VTOP(TARGET_TYPE_AR6003, 
				                 AR6003_HOST_INTEREST_ITEM_ADDRESS(hi_flash_is_present)),
                                 (A_UCHAR *)&param,
                                 4) != A_OK)	
        {
            AR_DEBUG_PRINTF(ATH_DEBUG_ERR,("SDIO_InitAR6000_ene failed at hi_flash_is_present \n"));
            return A_ERROR;
	    }
    }
...
static void parseRemoteIP_ini_SDIO(void)
{
	...
	while(fgets(lineBuf, sizeof(lineBuf), fStream) != NULL) {
		...
		else if(strncmp(DEF_SDIO_HOST_NVMEM_OVERRIDE, pLine, strlen(DEF_SDIO_HOST_NVMEM_OVERRIDE)) == 0) {
			pLine = strchr(pLine, '=');
			pLine++;
			pLine = strtok( pLine, delimiters ); //get past any white space etc
			
			if(!sscanf(pLine, "%i", &sdioNvmemOverride)) {
				printf("Unable to read the sdioNvmemOverride\n");
			}
		}
	}
}
...
int SDIO_BMITransferEepromFile_win(HANDLE handle, A_UCHAR *eeprom, A_UINT32 length)
{
    ...
    } else if (ar->arTargetType == TARGET_TYPE_MCKINLEY) {
        board_ext_data_size = MCKINLEY_BOARD_EXT_DATA_SZ;
    }
    ...
    {
        board_ext_data_size =  length - board_data_size;
