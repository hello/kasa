...
dhd_conf_read_config(dhd_pub_t *dhd)
{
	...
	pick = kzalloc(MAXSZ_BUF, GFP_KERNEL);
	if (!pick) {
		CONFIG_ERROR(("%s: Failed to allocate memory %d bytes\n",
	...
	if (len > 0 && len < MAXSZ_CONFIG) {
		...
		/* Process roam_cache */
		memset(pick, 0, MAXSZ_BUF);
		len_val = process_config_vars(bufp, len, pick, "roam_cache_ssid=");
		if (len_val) {
				memset(&dhd->conf->roam_cache_ssid, 0, 32);
				memcpy(dhd->conf->roam_cache_ssid, pick, len_val);
				printf("%s: roam_cache_ssid = %s\n", __FUNCTION__, dhd->conf->roam_cache_ssid);
				memset(pick, 0, MAXSZ_BUF);
				len_val = process_config_vars(bufp, len, pick, "roam_cache_channel=");
				if (len_val) {
						dhd->conf->roam_cache_channel = (int32)simple_strtol(pick, NULL, 10);
						printf("%s: roam_cache_channel = %d\n", __FUNCTION__,
							dhd->conf->roam_cache_channel);
				}
		}
