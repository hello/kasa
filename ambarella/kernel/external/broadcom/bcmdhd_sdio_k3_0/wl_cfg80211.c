	...
	struct cfg80211_connect_params *sme)
{
	...
	if (chan) {
		...
		WL_ERR(("channel cache channel (%d), chanspec_list (%d), %d channels\n", wl->channel,
			chanspec_list[0], chan_cnt));
#else
		wl->channel = 0;
#endif /* ROAM_CHANNEL_CACHE */
	}
	...
	bool aborted, bool fw_abort)
{
	...
	if (!wl->escan_info.ndev) {
		WL_DBG(("%s: wl->escan_info.ndev = %p\n", __FUNCTION__,
			wl->escan_info.ndev));
		return BCME_OK;
	} else if (wl->escan_info.ndev != ndev) {
