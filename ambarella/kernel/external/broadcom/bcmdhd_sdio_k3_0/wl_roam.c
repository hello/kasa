...
void init_roam(int ioctl_ver, dhd_pub_t *dhd)
{
#ifdef D11AC_IOTYPES
	if (ioctl_ver == 1) {
		/* legacy chanspec */
...
//	uint8 channel;
	...
	roam_cache[n_roam_cache].ssid_len = strlen(dhd->conf->roam_cache_ssid);// sizeof(ssid);
	//channel = 6;
	roam_cache[n_roam_cache].chanspec =
		(dhd->conf->roam_cache_channel <= CH_MAX_2G_CHANNEL ? band2G : band5G) | band_bw | dhd->conf->roam_cache_channel;
	memcpy(roam_cache[n_roam_cache].ssid, dhd->conf->roam_cache_ssid, strlen(dhd->conf->roam_cache_ssid));
						...
						const wlc_ssid_t *ssid, int ioctl_ver)
{
	...
	for (i = 0; i < n_roam_cache; i++) {
		...
		WL_INFO(("%s: cache channel %x, channel %x\n", __FUNCTION__, channels[0], ch));
		WL_INFO(("%s: cache ssid %s, ssid %s\n", __FUNCTION__, roam_cache[i].ssid, ssid->SSID));
