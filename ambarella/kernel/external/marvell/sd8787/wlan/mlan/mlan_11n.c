 ...
 *  @param priv         A pointer to mlan_private
 *  @param tid          TID
 *  @param peer_addr    A pointer to peer address
 */
t_void
wlan_send_delba_to_entry_in_txbastream_tbl(pmlan_private priv, t_u8 tid,
                                           t_u8 * peer_address)
{
    ...
    while (tx_ba_stream_tbl_ptr !=
        ...
        if (tx_ba_stream_tbl_ptr->ba_status == BA_STREAM_SETUP_COMPLETE) {
            if (((tid == DELBA_ALL_TIDS) || (tid == tx_ba_stream_tbl_ptr->tid))
                ...
                tx_ba_stream_tbl_ptr->ba_status = BA_STREAM_SETUP_INPROGRESS;
                wlan_send_delba(priv, tx_ba_stream_tbl_ptr->tid,
 ...
 */
t_void
wlan_send_delba_to_entry_in_reorder_tbl(pmlan_private priv, t_u8 tid,
                                        t_u8 * peer_address)
{
    pmlan_adapter pmadapter = priv->adapter;
    ...
    while (rx_reor_tbl_ptr != (RxReorderTbl *) & priv->rx_reorder_tbl_ptr) {
        if (rx_reor_tbl_ptr->ba_status == BA_STREAM_SETUP_COMPLETE) {
            if (((tid == DELBA_ALL_TIDS) || (tid == rx_reor_tbl_ptr->tid)) &&
                ...
                wlan_send_delba(priv, rx_reor_tbl_ptr->tid, rx_reor_tbl_ptr->ta,
                                0);
            }
        }
...
wlan_11n_ioctl_delba(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    PRINTM(MINFO,
           ...
           cfg->param.del_ba.direction, cfg->param.del_ba.tid,
           cfg->param.del_ba.peer_mac_addr[0],
           cfg->param.del_ba.peer_mac_addr[1],
           cfg->param.del_ba.peer_mac_addr[2],
           cfg->param.del_ba.peer_mac_addr[3],
           cfg->param.del_ba.peer_mac_addr[4],
           cfg->param.del_ba.peer_mac_addr[5]);

    if (cfg->param.del_ba.direction & DELBA_TX) {
        wlan_send_delba_to_entry_in_txbastream_tbl(pmpriv,
                                                   cfg->param.del_ba.tid,
                                                   cfg->param.del_ba.
                                                   peer_mac_addr);
    }
    if (cfg->param.del_ba.direction & DELBA_RX) {
        wlan_send_delba_to_entry_in_reorder_tbl(pmpriv, cfg->param.del_ba.tid,
                                                cfg->param.del_ba.
                                                peer_mac_addr);
    }
