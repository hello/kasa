...
{
	fprintf(stderr,
		"Usage: i2cset [-f] [-y] [-m MASK] I2CBUS CHIP-ADDRESS DATA-ADDRESS [VALUE] [DATA-MODE] [ADDR-MODE]\n"
		...
		"  ex: i2cset -y 0 0x6e 0x0058 0xdead ww\n");
	exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	if (vmask) {
		...
		case I2C_SMBUS_WORD_DATA:
			if (regAddrSize == 8)
				...
				oldvalue = Read16(file, address, daddress, 2);
			break;
		default:
			if (regAddrSize == 8)
				oldvalue = i2c_smbus_read_byte_data(file, daddress);
			else
				oldvalue = Read16(file, address, daddress, 1);
			break;
		}
	...
	case I2C_SMBUS_WORD_DATA:
		if (regAddrSize == 8)
			...
			res = Write16(file, address, daddress, value, 2);
		break;
	default: /* I2C_SMBUS_BYTE_DATA */
		if (regAddrSize == 8)
			res = i2c_smbus_write_byte_data(file, daddress, value);
		else
			res = Write16(file, address, daddress, value, 1);
		break;
	}
	if (res < 0) {
